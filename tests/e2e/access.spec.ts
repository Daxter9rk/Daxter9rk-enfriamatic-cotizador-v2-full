...
async function logout(page: Page) {
  ...
  if ((page.viewportSize()?.width ?? 1024) <= 720 && (await navigationButton.count()) > 0) {
...
test('login is usable at 360 px', async ({page}, testInfo) => {
  test.skip(testInfo.project.name !== 'mobile-360', 'Validación específica del proyecto móvil.');
  await page.goto('/');
